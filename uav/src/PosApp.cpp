...
#include "obsAvoid/obsAvoid_impl.h"
...
PosApp::PosApp(TargetController *controller): UavStateMachine(controller), behaviourMode(BehaviourMode_t::Default), vrpnLost(false) {
    ...
    Tab *Custom_Ctrl = new Tab(getFrameworkManager()->GetTabWidget(),"Custom Ctrl");



    // with this we create three vertical tabs 
    TabWidget *Custom_Ctrl_Widget = new TabWidget(Custom_Ctrl->NewRow(),"User Control");
    CustomGainsTab = new Tab(Custom_Ctrl_Widget,"Setup");
    CustomAnglesTab = new Tab(Custom_Ctrl_Widget,"Graphs 01 pf");
    CustomPositonTab = new Tab(Custom_Ctrl_Widget,"Graphs 02 pf");

    xCtrl = new xyBackstepping(CustomGainsTab->At(0,0),"xCtrl-BKSTP");
    xCtrl->UseDefaultPlot(CustomPositonTab->NewRow());    

    yCtrl = new xyBackstepping(CustomGainsTab->At(0,1),"yCtrl-BKSTP");
    yCtrl->UseDefaultPlot(CustomPositonTab->LastRowLastCol());

    z_Ctrl = new zBackstepping(CustomGainsTab->At(0,2),"zCtrl");
    z_Ctrl->UseDefaultPlot(CustomPositonTab->LastRowLastCol());

    attQuat = new attQuatBkstp(CustomGainsTab->At(1,1),"Attitude Quaternion");
    attQuat->UseDefaultPlot(CustomAnglesTab->LastRowLastCol());

    GroupBox *PosRef = new GroupBox(CustomGainsTab->At(1,0),"Position References");
    ...
    Tab *obstacle_tab = new Tab(getFrameworkManager()->GetTabWidget(),"Obstcle Tab");
    TabWidget *obsTab_widget = new TabWidget(obstacle_tab->NewRow(),"User Control");
    setup_obs = new Tab(obsTab_widget,"Setup");
    graph_obs_01 = new Tab(obsTab_widget,"Graphs 01 pf");
    graph_obs_02 = new Tab(obsTab_widget,"Graphs 02 pf");

    obs_01 = new obsAvoid(setup_obs->At(0,0),"Object 01");
    obs_01->UseDefaultPlot(graph_obs_01->LastRowLastCol());
...
void PosApp::xyControl(void){
    ...
    if(_ThrustMode==ThrustMode_t::Custom){
        ...
        if(SetThrustMode(ThrustMode_t::Custom)){
            ...
            cout << "NO se ha podido cambiar a TRUST CUSTOM" << endl; 
        }
    }
}
...
void PosApp::ComputePositionControllers(flair::core::Vector3Df &Controller){
   ...
   // float error = pos_z  - z_des->Value();
    ...
    obs_01->SetValues(uav_pos.x,uav_pos.y,uav_vel.x,uav_vel.y,x_des->Value(),y_des->Value());
    obs_01->Update(GetTime());


    float FobsX = obs_01->Output(0);
    //cout << "   FobsX: " << FobsX;
    float FobsY = obs_01->Output(1);
    //cout << "   FobsY: " << FobsY;
    ...
    Controller.x = ux + FobsX;
    Controller.y = uy + FobsY;
    Controller.z = uz;
}
